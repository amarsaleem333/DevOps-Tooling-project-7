...
2- Configure access to NFS for clients within the same subnet:
...
3- Firewall & Security Groups Entry:
Check which port is used by NFS and open it using Security Groups by adding a new Inbound Rule:

rpcinfo -p | grep nfs

Important Note: In order for the NFS server to be accessible from your client, you must open the following inbound ports: TCP 2049, UDP 2049, TCP 111, and UDP 111.

Step 2: Configure the Database Server
1- Connect to your Database Server and install MySQL:
...
2-Log into MySQL as root and run the following queries to configure the database and user permissions:
...
3-Configure MySQL to listen to remote connections by opening the configuration file:
sudo vi /etc/my.cnf
...
2- Create the target directory and mount the NFS server's shared application export to /var/www:

sudo mkdir -p /var/www
sudo mount -t nfs -o rw,nosuid 172.31.17.253:/mnt/apps /var/www

3- Create the Apache log directory and mount it to the NFS server's log export:
sudo mkdir -p /var/log/httpd
sudo mount -t nfs -o rw,nosuid 172.31.17.253:/mnt/logs /var/log/httpd

4-Make the mounts permanent so they persist after a reboot:
...
5- Install Apache, enable required repositories, and install PHP 7.4 along with its extensions:
...
Hard-Fought Fixes & Troubleshooting Highlights
